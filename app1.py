...
def validate_historical_accuracy(response, language="English"):
    anachronistic_terms = {
        ...
        "German": ["Amazon", "eBay", "internet", "E-Mail", "soziale Medien"],
    }
...
def ask():
    ...
    language = lang_map.get(detected_lang, 'English')  # Default to English

    context = retriever.get_relevant_documents(question)
    context_text = " ".join([doc.page_content for doc in context])
    
    result = chain({"question": question, "context": context_text, "chat_history": chat_history, "language": language})
    validated_answer = validate_historical_accuracy(result['answer'], language)
    
    # Update chat history with the latest interaction
    chat_history.append((question, validated_answer))
    
    # Debugging: Print chat_history to the console
